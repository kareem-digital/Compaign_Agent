...
Nodes that reach VOW - directly through MCP, or through the grounded registry - are factories
(`make_*`) taking what they need, so a test passes a fake in one line. `ask_for_missing` and
`plan_ready` reach nothing and are plain functions.
"""

from app.agent.nodes.ask_for_missing import ask_for_missing
from app.agent.nodes.extract_fields import make_extract_fields
